...
# compromise: 
    ...
    # if even number of channels, split cable into two even halves
    # if odd, split into even halves that both include middle channel
    split = num_chans - div(num_chans,2)
...
#         try
            ...
            if size(N.data)[1] == samples_per_file && isempty(N.data) == false
                ...
                if sgn in ["both","pos","neg"]
                    ...
                    # if u-shaped cable, apply fk filter spearately to each leg of the cable
                    if geometry == "u"
                        N_leg_1 = N[1:split]
                        ...
                        N_leg_1.data = N_leg_1.data[:,pad_bound+1:pad_bound+split]
                        N_leg_2.data = N_leg_2.data[:,pad_bound+1:pad_bound+split-1]
                        N_leg_1.n = split
                        N_leg_2.n = split-1
                        N = merge_channels(N_leg_1,N_leg_2,2)

                    # if linear cable, just fk the data
                    else
                        # pad with zeros
                ...
                elseif mode == "auto_cross"
                    corr = autocorrelate_cross(NF,Int64(maxlag*NF.fs[1]),split)
                end
